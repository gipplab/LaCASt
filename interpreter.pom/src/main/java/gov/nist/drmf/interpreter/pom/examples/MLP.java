package gov.nist.drmf.interpreter.pom.examples;/*
...
//        System.out.println(GlobalPaths.PATH_REFERENCE_DATA.toAbsolutePath().toString());
        ...
        try{
            ...
            eq = "P_n^{\\left(\\alpha,\\beta\\right)}\\left(\\cos\\left(a\\Theta\\right)\\right)";
...
//            }
    ...
    public static PomTaggedExpression clean( PomTaggedExpression mother ){
        Map<String, String> fs = mother.getNamedFeatures();
...
//        List<FeatureSet> l = mother.getRoot().getAlternativeFeatureSets();
        ...
        List<PomTaggedExpression> c = mother.getComponents();
        for ( PomTaggedExpression p : c )
            clean( p );
    ...
    final public static String NL = System.lineSeparator();

    static String prettyPrint( PomTaggedExpression mother, String space ){
        //System.out.println(mother.getRoot());
        //System.out.println(mother.getRoot().isEmpty());
        //System.out.println(mother.getComponents().size());

        if ( !mother.getRoot().isEmpty() )
            return space+mother.getRoot().getTermText()+NL;

        String rootStr = space;
        List<PomTaggedExpression> list = mother.getComponents();
        while ( !list.isEmpty() ){
            rootStr += prettyPrint(list.remove(0), SPACE+space)+NL;
        }
    ...
    static void printList(List<?> list) {
        for (Object elem : list){
            PomTaggedExpression pte = (PomTaggedExpression)elem;
            Map<String,String> features = pte.getNamedFeatures();
            for ( String feat : features.keySet() )
                pte.removeNamedFeature(feat);
