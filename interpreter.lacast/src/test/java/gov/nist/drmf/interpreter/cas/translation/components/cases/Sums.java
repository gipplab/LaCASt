...
public enum Sums implements ForwardTestCase {
    ...
    SIMPLE_LONG_LIMITS_UP(
            "\\sum_{n=0}^{m+1}{n}",
            "sum(n, n = 0..m + 1)",
            "Sum[n, {n, 0, m + 1}]"
    ),
    SIMPLE_LONG_LIMITS_LOW(
            "\\sum_{n= m-1}^{m+1}{n}",
            "sum(n, n = m - 1..m + 1)",
            "Sum[n, {n, m - 1, m + 1}]"
    ),
    ...
    NORM_MULTIPLE_SUMMANDS_HARD_FRAC(
            ...
            "Sum[r*Cos[\\[CapitalTheta]]*r*(3*(r)^(2) - 3)/23*x, {r, 0, 50}]+ 3*q"
    ),
    ...
    DLMF_MULTI_SUM_SPECIAL_FUNC_LONG( //10.23.17 all
            ...
            "Sum[Divide[(Divide[1,2]*z)^(k)*BesselJ[k, z],(k)!*(n - k)], {k, 0, n - 1}] + Sum[(-1)^(m)*Divide[(n + 2*m)*BesselJ[n + 2*m, z],m*(n + m)], {m, 1, Infinity}]"
    ),
    @DLMF("10.23.17")
